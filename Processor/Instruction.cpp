  ...
  { switch (opcode)
	...
	if (Proc.P.my_num()==0) {
      ...
      case GSUBML:
	   ...
	   Proc.write_S2(r[0],Sans2);
   #elif defined(EXT_NEC_RING)
	   Proc.get_S2_ref(r[0]).sub(Proc.read_S2(r[1]),Proc.read_C2(r[2]),Proc.P.my_num());
        #else
      ...
      case GSUBMR:
	   ...
	   Proc.write_S2(r[0],Sans2);
       #elif defined(EXT_NEC_RING)
	        Proc.get_S2_ref(r[0]).sub(Proc.read_C2(r[1]),Proc.read_S2(r[2]),Proc.P.my_num());
        #else
      ...
      case GSUBSI:
	   ...
	   Proc.write_S2(r[0],Sans2);
   #elif defined(EXT_NEC_RING)
	   Proc.get_S2_ref(r[0]).sub(Proc.read_S2(r[1]),Proc.temp.ans2,Proc.P.my_num());
        #else
        ...
        if (Proc.P.my_num() == 0)
          {
        	...
        	for(int b = 0; b<BATCH_SIZE; b++)
        	{
				...
				uint64_t v = Proc.read_Cp(start[0]).get_ring(b);
				uint64_t p = Proc.read_Cp(start[1]).get_ring(b);
				uint64_t z = Proc.read_Cp(start[2]).get_ring(b);
				uint64_t s = Proc.read_Cp(start[3]).get_ring(b);
        ...
        if (Proc.P.my_num() == 0)
          {
        	for(int b = 0; b<BATCH_SIZE; b++)
        	{
        		uint64_t tmp_val = Proc.read_Cp(r[0]).get_ring(b);
         		uint64_t val;
         		double res =0;
         		double int_part =0;
         		double dec_part =0;
